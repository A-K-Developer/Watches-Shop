...
import { animateElementOnScroll,  checkWatches, eventAndPrint, eventOnBtn, fixedNavBar, readMoreOPT,  } from './javascript';
import { articleTemplate, footerTemplate, productDescription, templateForHowItWorks, templateForMonthlyWatch, templateForNav, templateForSlideshow } from './templates';
...
const firebaseConfig = {
  ...
  initializeApp(firebaseConfig)
    ...
    let priviousWatches = doc(db,'Watchess','January');
    onSnapshot(priviousWatches, (doc) => {
        let data = doc.data();
        checkWatches(data);
        eventAndPrint(data);
    })
    let priviousWatches1 = doc(db,'Watchess','February');
    onSnapshot(priviousWatches1, (doc) => {
        let data = doc.data();
        checkWatches(data);
        eventAndPrint(data);
    })
    let priviousWatches2 = doc(db,'Watchess','March');
    onSnapshot(priviousWatches2, (doc) => {
        let data = doc.data();
        checkWatches(data);
        eventAndPrint(data);
    })
    let priviousWatches3 = doc(db,'Watchess','April');
    onSnapshot(priviousWatches3, (doc) => {
        let data = doc.data();
        checkWatches(data);
        eventAndPrint(data);
    })
